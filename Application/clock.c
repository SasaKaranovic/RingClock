...
volatile ws_led_t BuffClock[LED_COUNT]          = { {.RED = 0x00,        .GREEN = 0x00,         .BLUE = 0x00 } };   // Buffer for holding each LED value
...
uint8_t colorMinutes_fadeInSteps = 20;
...
void Clock_DrawTime(uint8_t ClockHour, uint8_t ClockMinute)
{
    ...
    //Mark elapsed minutes with newely chosen
